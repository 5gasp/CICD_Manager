...
async def create_service_test(serviceTest:UploadFile = File(...) , db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not success:
            return Utils.create_response(status=400, success=False, message=f"{response}", data=[])
    except Exception as e:
        return Utils.create_response(status=400, success=False, message=f"{e}", data=[])


    #3 ->  Get the attachment (testing descriptor)
   
    try:
        attachment_url = response['data']['attachment'][0]["url"]
